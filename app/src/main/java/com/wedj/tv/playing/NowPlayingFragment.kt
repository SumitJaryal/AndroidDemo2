...
//    private lateinit var database: TvMediaDatabase
    ...
    private inner class MediaPlayerGlue(context: Context, adapter: LeanbackPlayerAdapter) :
        ...
        fun setMetadata(metadata: ManageVideoResponse) {
            // Displays basic metadata in the player
            title = metadata.titles
            subtitle = metadata.fullName
            lifecycleScope.launch(Dispatchers.IO) {
                metadata.vdid.let {
                    art = Coil.get(Uri.parse(getVideoBannerUrl(it)))
                }
            }
            ...
            val url =
                "https://www.learningcontainer.com/wp-content/uploads/2020/05/sample-mp4-file.mp4"//"https://www.youtube.com/watch?v="+metadata.vdid
...
//        database = TvMediaDatabase.getInstance(requireContext())
        val metadata = arguments?.getSerializable(Constants.ARGUMENT_VIDEO)
            ...
            addPlayerCallback(object : PlaybackGlue.PlayerCallback() {
                ...
                override fun onPreparedStateChanged(glue: PlaybackGlue?) {
                    ...
                    if (glue?.isPrepared == true) {
                        ...
                        seekTo(startingPosition.toLong())
                    }
                }
...
//                    TvLauncherUtils.removeFromWatchNext(requireContext(), args.metadata)
            ...
            setMetadata(metadata as ManageVideoResponse)
        }
